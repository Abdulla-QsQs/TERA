...
The public beta is hosted by Cloudflare Pages, Functions, and D1. Cloudflare processes ordinary network metadata, including IP addresses, under its own policies. TERA passes the connecting IP only to Cloudflare's short-lived rate-limiter binding and does not write it to D1. The application-level D1 retention process removes display names after 90 days and visitor/compilation records after 24 months.
